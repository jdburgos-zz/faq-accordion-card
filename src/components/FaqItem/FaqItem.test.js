...
describe('FaqItems component', () => {
  ...
  test("doesn't render content text if the item wasn't clicked", () => {
    render(<FaqItem item={ mockItem } />);
    const contentElement = screen.queryByText(/You can invite up to 2 additional users on the Free plan. There is no limit on team members for the Premium plan./i);
  ...
  test("renders content text if the item was clicked", () => {
    ...
    const contentElement = screen.getByText(/You can invite up to 2 additional users on the Free plan. There is no limit on team members for the Premium plan./i);
