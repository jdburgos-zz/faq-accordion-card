...
const items = ITEMS.map((item, index) => <li key={ index }><FaqItem item={ item } /></li>);

const FaqItems = () => (
  <div className={styles['faq-items']}>
    <h1 className={styles['faq-items__title']}>FAQ</h1>
    <ul className={styles['faq-items__items']}>{items}</ul>
  </div>
);
